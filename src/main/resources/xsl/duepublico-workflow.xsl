...
>
  <xsl:import href="xslImport:modsmeta:mir-workflow.xsl" />
  
  <xsl:param name="WebApplicationBaseURL" />
  
  <xsl:template match="/">
    <xsl:apply-templates select="mycoreobject" mode="mir.workflow" />
    <xsl:apply-imports />
  </xsl:template>

  <xsl:template match="mycoreobject" mode="mir.workflow">
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        ...
        <div class="row detail_row" id="mir-workflow">
          <div class="col-md-12">
            <div class="row">
              <div class="col-md-12">
                <div class="alert alert-info" role="alert">
                  <ul style="list-style-type:none; padding:0;">
                    ...
                    <xsl:call-template name="mir.workflow.step">
  ...
  <xsl:template name="mir.workflow.step">
    <xsl:param name="step" />
    <xsl:param name="icon" />
    ...
    <li>
      ...
      <div style="display:inline-block; vertical-align:top;">
        <xsl:value-of select="i18n:translate(concat('mir.workflow.step.',$step))" />:
        <a class="alert-link" href="{$link}">
          <xsl:value-of select="i18n:translate(concat('mir.workflow.step.',$step,'.',$checked))" />
        </a>
      </div>
    </li>
  </xsl:template>
